...
**対象アプリ**: `GLASS/synthesizer_app/`（GLASS LASブランチを利用したNGデータ合成GUIアプリ）
...
`glass_env` の Python から `synthesizer_app.gui_main` を起動します。

```bash
# GLASS/ ディレクトリに cd してから
"C:/Users/seong/anaconda3/envs/glass_env/python.exe" -m synthesizer_app.gui_main
```

または、`GLASS/synthesizer_app/run.bat` をダブルクリックしても起動できます。
