...
- `GLASS/synthesizer_app/` — **NG-data synthesis GUI app**, the long-term deliverable. Lives inside the GLASS clone for development convenience but is **also published as a standalone public repo** (see "Standalone publication" below).
  ...
  - `tests/_capture_manual_screens.py` — doc utility (NOT a unit test). Launches the GUI with `-topmost`, drives tab switches and a Preview synthesis programmatically, and writes 7 PNGs to `00.docs/manual_screens/`. Re-run after any UI change to refresh the manual's screenshots. Underscore prefix excludes it from `unittest discover`.
...
- `00.docs/` — reference PDF of the GLASS paper (`2407.09359v1.pdf`, ECCV 2024), `GLASS_synth_gui_plan.md` (the GUI-app implementation plan with phase breakdown and confirmed decisions), `GLASS_synthesizer_user_manual.md` (Japanese end-user manual covering every widget and typical workflows, with embedded screenshots), and `manual_screens/*.png` (the 1600×900 captures referenced from that manual; regenerated by `synthesizer_app/tests/_capture_manual_screens.py`).
...
End-user documentation lives at `00.docs/GLASS_synthesizer_user_manual.md` (Japanese; covers every widget on the Control / Configure tabs, the preview area, the thumbnail strip, and typical workflows, with screenshots from `00.docs/manual_screens/`). When you change widget behavior, layout, or labels, **also re-run `synthesizer_app/tests/_capture_manual_screens.py`** to refresh those PNGs — the script preset paths are pinned to this workstation's MVTec/DTD locations so it works out of the box here.
